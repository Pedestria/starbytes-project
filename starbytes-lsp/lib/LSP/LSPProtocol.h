...
TYPED_ENUM LSPDiagonisticLevel : int {
  Error = 1,
  Warning = 2,
  Information = 3,
  Hint = 4
...
TYPED_ENUM LSPDiagonisticTag : int { Unnecessary = 1, Deprecated = 2 };
...
TYPED_ENUM LSPDocumentChangeType : int {
  LSPCreateFile,
  LSPRenameFile,
  LSPDeleteFile,
  LSPTextDocumentEdit
...
enum class LSPTextDocumentIDType : int { Regular, Versioned };

struct LSPTextDocumentIdentifier : LSPServerObject {
  LSPTextDocumentIDType id_type;
  LSPDocumentURI uri;
  bool operator == (LSPTextDocumentIdentifier &other) {
    return (other.id_type == id_type) && (other.uri == uri);
  };
};
...
TYPED_ENUM LSPResourceOperationKind : int { Create, Rename, Delete };

TYPED_ENUM LSPFailureHandlingKind : int {
  Abort,
  Transactional,
  Undo,
...
enum class LSPMarkupKind : int { PlainText, Markdown };
...
enum class LSPWorkDoneProgressParamsType : int { IntializeParams };
...
enum class LSPTraceSetting : int { Off, Messages, Verbose };
...
enum class LSPWatchKind : int { Create = 1, Change = 2, Delete = 4 };
...
enum class LSPFileChangeType : int { Created = 1, Changed = 2, Deleted = 3 };
...
struct LSPWorkspaceSymbolParams : LSPWorkDoneProgressParams,
...
enum class LSPTextDocumentSyncKind : int { None, Full, Incremental };
...
enum class LSPTextDocumentSaveReason : int { Manual = 1, AfterDelay, FocusOut };
...
struct LSPCompletionParams : LSPTextDocumentPositionParams,
...
enum class LSPCompletionItemTag : int { Deprecated = 1 };

enum class LSPInsertTextFormat : int { PlainText = 1, Snippet = 2 };
...
struct LSPCompletionRegistrationOptions : LSPTextDocumentRegistrationOptions,
...
struct LSPHoverClientCapabilities : LSPServerObject {
  bool dynamic_registration;
  std::vector<LSPMarkupKind> content_format;
};

struct LSPHoverOptions : LSPWorkDoneProgressOptions {};

struct LSPHoverRegistrationOption : LSPTextDocumentRegistrationOptions,
                                    LSPHoverOptions {};

struct LSPHoverParams : LSPWorkDoneProgressParams,
                        LSPTextDocumentPositionParams {};

struct LSPHover : LSPServerObject {
...
enum class LSPIntializeErrorCode : int { UnknownProtocolVersion = 1 };
